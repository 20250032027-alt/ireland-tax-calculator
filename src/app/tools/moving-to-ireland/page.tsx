    ...
    <>
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px" }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <h1 style={{ fontSize: 36, fontWeight: 700, color: "#f0f4ff", marginBottom: 12 }}>Moving to Ireland: Tax Guide 2026</h1>
        ...
        <div style={{ background: "rgba(0,168,107,0.08)", border: "1px solid rgba(0,168,107,0.2)", borderRadius: 20, padding: 28, marginBottom: 48 }}>
          <p style={{ fontSize: 15, fontWeight: 600, color: "#f0f4ff", marginBottom: 16 }}>New arrival checklist</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }} className="calc-grid">
            {checklist.map(item => (
              <div key={item} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                <CheckCircle size={14} style={{ color: "#00a86b", flexShrink: 0, marginTop: 2 }} />
                <span style={{ fontSize: 13, color: "#8899bb", lineHeight: 1.5 }}>{item}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 40 }}>
          {sections.map(({ id, title, content }) => (
            <div key={id}>
              <h2 style={{ fontSize: 22, fontWeight: 700, color: "#f0f4ff", marginBottom: 16, paddingBottom: 12, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                {title}
              </h2>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {content.map((para, i) => (
                  <p key={i} style={{ fontSize: 14, color: "#8899bb", lineHeight: 1.8 }}>{para}</p>
                ))}
              </div>
            </div>
          ))}
        ...
        <div style={{ marginTop: 48, background: "#111d35", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, overflow: "hidden" }}>
          <div style={{ padding: "18px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "#0f1f3d" }}>
            <p style={{ fontSize: 15, fontWeight: 600, color: "#f0f4ff" }}>Ireland vs UK vs US — quick comparison</p>
          </div>
          ...
          {[
            ...
            <div key={feature} style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr 1fr", padding: "12px 20px", borderBottom: "1px solid rgba(255,255,255,0.04)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)" }}>
              <span style={{ fontSize: 13, color: "#8899bb" }}>{feature}</span>
              <span style={{ fontSize: 13, color: "#00d084", fontWeight: 500 }}>{ireland}</span>
              <span style={{ fontSize: 13, color: "#f0f4ff" }}>{uk}</span>
              <span style={{ fontSize: 13, color: "#f0f4ff" }}>{us}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginTop: 48, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }} className="calc-grid">
          {[
            ...
            <Link key={href} href={href} style={{ display: "block", padding: 24, borderRadius: 16, background: "#111d35", border: "1px solid rgba(255,255,255,0.08)", textDecoration: "none" }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: "#f0f4ff", marginBottom: 6 }}>{title}</p>
              <p style={{ fontSize: 13, color: "#8899bb", marginBottom: 16 }}>{desc}</p>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 600, color: "#00a86b" }}>
