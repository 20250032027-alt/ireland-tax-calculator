  ...
  return (
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        <div className="mb-10">
          <div className="inline-flex items-center gap-2 text-xs px-3 py-1.5 rounded-full mb-4" style={{ background: "rgba(0,168,107,0.12)", border: "1px solid rgba(0,168,107,0.25)", color: "var(--accent-green-light)" }}>
            Income Tax
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="rounded-2xl p-6" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
            <h2 className="text-sm font-semibold mb-5" style={{ color: "var(--text-primary)" }}>Desired take-home</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm mb-1.5" style={{ color: "var(--text-secondary)" }}>Target net annual salary</label>
            ...
            {result && gross > 0 && (
              <>
                <div className="rounded-2xl p-6" style={{ background: "rgba(0,168,107,0.08)", border: "1px solid rgba(0,168,107,0.25)" }}>
                  <p className="text-xs mb-1" style={{ color: "var(--text-secondary)" }}>Gross salary required</p>
                  <p className="text-4xl font-bold" style={{ color: "var(--accent-green-light)" }}>{formatEuro(gross)}</p>
                  <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>{formatEuro(gross / 12)} per month gross</p>
                </div>

                <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
                  <div className="px-5 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                    <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Deductions at that salary</p>
                  </div>
                  {[
                    ...
                    <div key={label} className="flex justify-between px-5 py-2.5 border-b text-sm" style={{ borderColor: "var(--border)" }}>
                  ...
                  <div className="flex justify-between px-5 py-3" style={{ background: "var(--bg-secondary)" }}>
                    <span className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Take-home</span>
                    <span className="text-sm font-bold font-mono" style={{ color: "var(--accent-green-light)" }}>{formatEuro(result.net)}</span>
                  </div>
                </div>
              </>
            )}
