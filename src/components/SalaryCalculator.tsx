...
const S = {
  card: { background: "var(--bg-2)", border: `1px solid var(--border-0)`, borderRadius: 20, padding: 24 } as React.CSSProperties,
...
function MetricCard({ label, value, sub, highlight }: { label: string; value: string; sub?: string; highlight?: boolean }) {
  return (
    <div style={{
      ...
      <p style={{ fontSize: 12, color: "var(--text-1)", marginBottom: 4 }}>{label}</p>
      <p style={{ fontSize: 22, fontWeight: 700, color: highlight ? "#00d084" : "#f0f4ff", ...S.mono }}>{value}</p>
      {sub && <p style={{ fontSize: 11, color: "var(--text-2)", marginTop: 3 }}>{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ ...S.row, marginBottom: 5 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-0)", ...S.mono }}>{formatEuro(amount)}</span>
      </div>
...
export default function SalaryCalculator() {
  ...
  const CheckboxRow = ({ checked, onChange, title, sub }: { checked: boolean; onChange: (v: boolean) => void; title: string; sub: string }) => (
    <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer" }}>
      ...
      <div>
        <span style={{ fontSize: 13, color: "var(--text-0)", display: "block" }}>{title}</span>
        <span style={{ fontSize: 11, color: "var(--text-2)" }}>{sub}</span>
      </div>
    </label>
  );
  ...
  return (
    <div className="calc-grid">
      {/* LEFT */}
      <div style={S.card}>
        <p style={{ fontSize: 15, fontWeight: 600, color: "var(--text-0)", marginBottom: 20 }}>Your details</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ ...S.row, marginTop: 4 }}>
              <span style={{ fontSize: 11, color: "var(--text-2)" }}>0%</span>
              <span style={{ fontSize: 11, color: "var(--text-2)" }}>40% max</span>
            </div>
          </div>

          {/* Optional credits */}
          <div style={{ background: "var(--bg-1)", border: `1px solid var(--border-0)`, borderRadius: 12, padding: 16 }}>
            <p style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-2)", marginBottom: 14 }}>Optional tax credits</p>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginTop: 20 }}>
          {["Revenue.ie rates", "Budget 2026", "No registration"].map(t => (
            <div key={t} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 12, color: "var(--text-1)" }}>{t}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "14px 20px", borderBottom: `1px solid var(--border-0)` }}>
            <p style={{ fontSize: 14, fontWeight: 600, color: "var(--text-0)" }}>Annual breakdown</p>
          </div>
          {[
          ...
          <div style={{ ...S.row, padding: "14px 20px", background: "var(--bg-1)" }}>
            <span style={{ fontSize: 14, fontWeight: 700, color: "var(--text-0)" }}>Take-home pay</span>
            <span style={{ fontSize: 20, fontWeight: 700, color: "var(--accent-hi)", ...S.mono }}>{formatEuro(result.net)}</span>
          </div>
        </div>
        ...
        {showBands && (
          <div style={{ ...S.card }}>
            <p style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-2)", marginBottom: 16 }}>Income tax bands</p>
            ...
            <p style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--text-2)", margin: "20px 0 16px" }}>USC bands</p>
            {result.uscBands.length === 0
              ? <p style={{ fontSize: 13, color: "var(--text-1)" }}>Exempt — income under €13,000</p>
